...
def make_plots(method="cubic_ns", n_steps=5, prefix="polar_maps"):
    ...
    def run_label(run_method, run_steps):
        ...
        if run_method == "quintic_ns":
            return f"Quintic Newton-Schulz ({run_steps} iters)"
    ...
    def draw_plot(x, axis_mode="linear", xlim=None, ylim=None, title="", outpath="plot.svg", y_floor=None):
        ...
        for idx, run in enumerate(ns_runs):
            ...
            plot_fn(
                ...
                linewidth=2.5,
                label=run["label"],
                color=ns_shades[idx % len(ns_shades)],
            )
    ...
    logx_path = f"./tikh_ns_polar_logx.svg"
    loglog_path = f"./tikh_ns_polar_loglog.svg"
    ...
    draw_plot(
        ...
        xlim=(1e-4, 1),
        ylim=(0, 1.1),
        title=f"Quintic NS and Tikhonov Regularised Polar Maps (Log x-axis)",
        outpath=logx_path,
    )
    ...
    draw_plot(
        ...
        xlim=(1e-4, 1),
        ylim=(1e-4, 2),
        title=f"Quintic NS and Tikhonov Regularised Polar Maps (Log-Log)",
        outpath=loglog_path,
...
if __name__ == "__main__":
    ...
    make_plots(method="quintic_ns", n_steps=[1,3,5], prefix="polar_maps")
